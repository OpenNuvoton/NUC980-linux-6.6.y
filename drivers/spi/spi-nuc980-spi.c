...
static int nuvoton_spi_probe(struct platform_device *pdev)
{
	...
	*/
	...
	if (val32 == (u32)0xb0061000)
		nuvoton->clk = clk_get(NULL, "spi0_eclk");
	else
		nuvoton->clk = clk_get(NULL, "spi1_eclk");
	if (IS_ERR(nuvoton->clk)) {
	...
	if (val32 == (u32)0xb0061000)
		nuvoton->clk = clk_get(NULL, "spi0");
	else
		nuvoton->clk = clk_get(NULL, "spi1");
	if (IS_ERR(nuvoton->clk)) {
		dev_err(&pdev->dev, "No clock for device\n");
		err = PTR_ERR(nuvoton->clk);
		goto err_clk;
...
MODULE_DESCRIPTION("Nuvoton NUC980 SPI Controller driver!");
