...
{
	struct nuvoton_spi *hw = spi_controller_get_devdata(mem->spi->controller);
	unsigned int max_buswidth = hw->pdata->quad ? 4 : 1;

	if (op->data.buswidth > max_buswidth || op->addr.buswidth > max_buswidth ||
	    op->dummy.buswidth > max_buswidth || op->cmd.buswidth > max_buswidth)
		return false;
