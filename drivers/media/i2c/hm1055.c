...
static int hm1055_probe(struct i2c_client *client)
{
	...
	sensor->current_mode = &hm1055_mode_data[HM1055_MODE_VGA_640_480];
	...
	of_property_read_u32_array(dev->of_node,"port-number", &port_number,1);
	...
	if(port_number == 0) {
		...
		if (IS_ERR(clkmux)) {
			printk(KERN_ERR "nuc980-ccap0:failed to get cap clock source\n");
			ret = PTR_ERR(clkmux);
			return ret;
		}
		...
		if (IS_ERR(clkcap)) {
			printk(KERN_ERR "nuc980-ccap1:failed to get cap clock source\n");
			ret = PTR_ERR(clkcap);
			return ret;
		}
